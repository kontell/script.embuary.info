...
DISCOVER_INDEX = {
    "movie": [
        ...
        {
            "name": ADDON.getLocalizedString(32053),
            "option": "genre",
            "param": "with_genres",
        },
    ],
    "tv": [
        ...
        {
            "name": ADDON.getLocalizedString(32054),
            "option": "year",
            "parmam": "first_air_date_year",
        },
        {
            "name": ADDON.getLocalizedString(32055),
            "option": "genre",
            "param": "with_genres",
        },
    ],
...
def index():
    for i in ["discover", "movie", "tv", "nextaired", "search"]:
        ...
        xbmcplugin.addDirectoryItem(
            plugin.handle, plugin.url_for(eval(item["route"])), li_item, item["folder"]
        )
...
def nextaired(day=None):
    if not day:
        for i in INDEX_MENU["nextaired"].get("menu"):
            ...
            li_item.setArt(DEFAULT_ART)
            xbmcplugin.addDirectoryItem(
                plugin.handle, plugin.url_for(nextaired, i.get("day")), li_item, True
            )

        utc = arrow.utcnow()
        local_date = utc.to(TIMEZONE)

        kodi_locale = json_call(
            "Settings.GetSettingValue", params={"setting": "locale.language"}
        )
        kodi_locale = kodi_locale["result"]["value"][-5:]

        for i in range(6):
            local_date = local_date.shift(days=1)
            translated_date = local_date.format(
                fmt="dddd, D. MMMM YYYY", locale=kodi_locale
            )
            ...
            li_item.setArt(DEFAULT_ART)
            xbmcplugin.addDirectoryItem(
                plugin.handle, plugin.url_for(nextaired, tmp_day), li_item, True
            )
    ...
    for i in next_aired_results:
        try:
            ...
            li_item.setInfo(
                "video",
                {
                    "title": i.get("name") or xbmc.getLocalizedString(13205),
            ...
            xbmcplugin.addDirectoryItem(
                plugin.handle,
                plugin.url_for(dialog, "tv", "tmdb", i["show_id"]),
                li_item,
            )
...
def discover(directory=None, option=None, filterby=None, page=1, pages=1):
    if not directory:
        for i in INDEX_MENU["discover"].get("menu"):
            ...
            li_item.setArt(DEFAULT_ART)
            xbmcplugin.addDirectoryItem(
                plugin.handle, plugin.url_for(discover, i.get("call")), li_item, True
            )

        _category(category=INDEX_MENU["discover"]["name"])

    else:
        category = _dict_match(
            "name", INDEX_MENU["discover"]["menu"], "call", directory
        )
        ...
        if directory == "person":
            ...
            filter_param = _dict_match(
                "param", DISCOVER_INDEX[directory], "option", option
            )
            result, pages = _query(
                "discover", directory, params={filter_param: filterby, "page": page}
            )
...
def _discover_option(call, option):
    if option == "genre":
        tmdb = tmdb_query(action="genre", call=call, get="list")
...
def _listing(directory, call, page, pages):
    ...
    if not call:
        ...
        for i in INDEX_MENU[directory].get("menu"):
            ...
            li_item.setArt(DEFAULT_ART)
            xbmcplugin.addDirectoryItem(
                plugin.handle, plugin.url_for(eval(route), i.get("call")), li_item, True
            )
    ...
    if call == "tv":
        for item in items:
            list_item, is_local = tmdb_handle_tvshow(
                item, local_items=local_items.get("shows", [])
            )
            xbmcplugin.addDirectoryItem(
                plugin.handle,
                plugin.url_for(dialog, "tv", "tmdb", item["id"]),
                list_item,
            )
        ...
        for item in items:
            list_item, is_local = tmdb_handle_movie(
                item, local_items=local_items.get("movies", [])
            )
            xbmcplugin.addDirectoryItem(
                plugin.handle,
                plugin.url_for(dialog, "movie", "tmdb", item["id"]),
                list_item,
            )
        ...
        for item in items:
            ...
            xbmcplugin.addDirectoryItem(
                plugin.handle,
                plugin.url_for(dialog, "person", "tmdb", item["id"]),
                list_item,
            )
...
def _query(content_type, call, get=None, params=None, get_details=False):
    ...
    if not tmdb:
        tmdb = tmdb_query(action=content_type, call=call, get=get, params=args)
...
def _previouspage(page):
    if int(page) > 1 and condition(
        "Window.IsVisible(MyVideoNav.xml) + !Container.HasParent"
    ):
        return True
    return False
