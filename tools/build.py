...
zip file" expects. Development-only files are left out so the zip carries only
what the addon needs at runtime. The id and version come straight from
``addon.xml``.

One exclusion is specific to this fork: upstream keeps the Photoshop sources
for the icon and fanart next to the exported PNG/JPG (``resources/icon.psd``,
``resources/fanart.psd``). They were kept out of upstream's release tarballs by
``.gitattributes export-ignore``, which only applies to ``git archive`` — this
build copies the working tree, so it has to exclude them itself. They are ~9 MB,
which is several times the rest of the addon.
...
# Repo-root entries that are development-only.
EXCLUDE_TOP = {
    "docs",
    "tests",
    "tools",
...
# Suffixes never shipped: byte-compiled Python, and the image sources above.
EXCLUDE_SUFFIX = (".pyc", ".pyo", ".psd")
